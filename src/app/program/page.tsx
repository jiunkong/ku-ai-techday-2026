const Row = (props: { start: string, end: string, program: string, organizer?: string, note?: string, center?: boolean, tag?: { text: string, color: "blue" | "green" } }) => {
    const tagColorClass = props.tag?.color === "blue" ? "text-blue-600" : "text-green-600";
    ...
    return (
        <>
            <tr className={`${props.center ? "h-10" : "h-20"} lg:h-20`}>
                ...
                <td className={`hidden lg:table-cell pl-2 text-lg font-bold ${props.center ? "text-center text-(--title)" : ""}`}>
                    { props.tag && <span className={`${tagColorClass} mr-2`}>{props.tag.text}</span> }
                    {props.program}
                </td>
                ...
                <td className="lg:hidden">
                    <div className={`font-bold ${props.center ? "text-(--title)" : ""}`}>
                        { props.tag && <span className={`${tagColorClass} mr-1.5`}>{props.tag.text}</span> }
                        {props.program}
                    </div>
                     { props.organizer && 
                        <div>
        ...
        <div className="w-screen pt-30 text-black">
            ...
            <table className="mx-auto mt-10 border-separate border-spacing-x-4 table-fixed pb-40">
                ...
                <tbody>
                    ...
                    <Row start="13:30" end="14:10" program="로봇 파운데이션 (RFM) 모델의 현재와 미래, 그리고 우리의 기회" organizer="신진우 교수" note="KAIST" tag={{ text: "키노트", color: "blue" }}/>
                    ...
                    <Row start="14:50" end="15:20" program="Beyond Locomotion: Turning Human Data into Humanoid Interaction Skills" organizer="Umar Iqbar" note="NVIDIA" tag={{ text: "온라인", color: "green" }}/>
